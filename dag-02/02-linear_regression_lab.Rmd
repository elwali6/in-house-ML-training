...
A "unit" increase in `TV` ad spending is associated with a `r round(coef(lrmod)[2],4)` "unit" increase in sales.
Or more clearly: An additional $1,000 spent on `TV` ads is associated with an increase in `sales` of `r round(1000*coef(lrmod)[2],2)` widgets.
...
```{r}
options( warn = 1)
coef.lrmod <- coef(lrmod)
ggplot() + geom_point(data = Advertising,aes(TV,sales), color = "blue") +     geom_abline(intercept=coef.lrmod[1],slope=coef.lrmod[2])
```
