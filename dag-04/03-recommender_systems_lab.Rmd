---
title: "Lab: Building a Movie Recommendation System"
author: ""
date: "`r Sys.Date()`"
output:
  html_notebook:
...
In this Lab, we develop a collaborative filtering recommender (CFR) system for recommending movies. 
...
Now, we create a *search matrix* which allows an easy search of a movie by any of its genre.
...
In order to use the ratings data for building a recommendation engine with *recommenderlab*, we convert rating matrix into a sparse matrix of type *realRatingMatrix*.

```{r rat_mat, echo=TRUE, error=FALSE, warning=FALSE}
...
* k: In the first step, the algorithm computes the similarities among each
pair of items. Then, for each item, it identifies its k most similar items
and stores it.

* method: This is the similarity function. By default, it is `Cosine`. Another
...
function. The supported methods to compute similarities are *cosine* and  *pearson*.

Next, we determine how similar the first four users are with each other by creating and visualizing similarity matrix that uses the cosine distance:
...
Using the same approach, we compute similarity between the first four movies.
...
Now we identify the top-rated movies by computing the average rating of each of them.
...
The first figure above shows the distribution of the average movie rating. The highest value is around 3, and there are a few movies whose average rating is either 1 or 5. Probably, the reason is that these movies received a rating from a few people only, so we shouldn't take them into account. 

we remove the movies whose number of views is below a defined threshold of 50, creating a subset of only relevant movies. The second figure above shows the distribution of the relevant average ratings. All the rankings are between 2.16 and 4.45. As expected, the extremes were removed. The highest value changes, and now it is around 4.

### Heatmap of the rating matrix

we visualize the whole matrix of ratings by building a heat map whose colors represent the ratings. Each row of the matrix corresponds to a user, each column to a movie, and each cell to its rating.
...
Some users saw more movies than the others. So, instead of displaying some random users and items, we select the most relevant users and items. Thus we visualize only the users who have seen many movies and the movies that have been seen by many users. To identify and select the most relevant users and movies, we follow these steps:
...
In order to select the most relevant data, we define the minimum number of users per rated movie as 50 and the minimum views number per movie as 50:

```{r rel_data}
ratings_movies <- ratingmat[rowCounts(ratingmat) > 50,
                             colCounts(ratingmat) > 50]
ratings_movies
ratings_movies@data[1:5,1:10]
#ratingmat
```
...
Using the same approach as previously, we visualize the top 2 percent of users and movies in the new matrix of the most relevant data:
...
Having users who give high (or low) ratings to all their movies might bias the results. In order to remove this effect, we normalize the data in such a way that the average rating of each user is 0. As a quick check, we calculate the average rating by users, and it is equal to 0, as expected:
...
Now, we visualize the normalized matrix for the top movies. It is colored now because the data is continuous:
...
is that we are visualizing only the top movies. we have already checked that the average rating is 0 for each user.
...
Now, it is possible to recommend movies to the users in the test set. we define
...
It's also possible to define a matrix with the recommendations for each user. we visualize the recommendations for the first four users:
...
Now, we will use the user-based approach. According to this approach, given a new user, its similar users are first identified. Then, the top-rated items rated by
...
Again, let's first check the default parameters of UBCF model. Here, *nn* is a number of similar users, and *method* is a similarity function, which is *cosine* by default. we build a recommender model leaving the parameters to their defaults and using the training set.
...
we also compute how many times each movie got recommended and build the related frequency histogram:
...
However, UBCF's accuracy is proven to be slightly more accurate than IBCF (we will also discuss it in the next section), so it's a good option if the dataset is not too big.
...
recommendations. For this, we first check the minimum number of items rated by users to be sure there will be no users with no items to test.
...
First, we re-define the evaluation sets, build IBCF model and create a matrix with predicted ratings.
...
Now, we compute the accuracy measures for each user. Most of the RMSEs (Root mean square errors) are in the range of 0.5 to 1.8:
...
In order to have a performance index for the whole model, we specify *byUser* as FALSE and compute the average indices:
...
*evaluate* function in *recommenderlab* library. The function evaluate the recommender performance depending on the number *n* of items to recommend to each user. we use *n* as a sequence n = seq(10, 100, 10). The first rows of the resulting performance matrix is presented below:
...
In order to have a look at all the splits at the same time, we sum up the indices of columns TP, FP, FN and TN:
...
Finally, we plot the ROC and the precision/recall curves:
...
Then, we define a different set of numbers for recommended movies (n_recommendations <- c(1, 5, seq(10, 100, 10))), run and evaluate the models:
...
we compare the models by building a chart displaying their ROC curves and Precision/recall curves.
...
IBCF takes account of the k-closest items. we will explore more values, ranging
...
Now we build and evaluate the same IBCF/cosine models with different values of the k-closest items:
...
In this Lab, we have developed and evaluated a collaborative filtering recommender (CFR) system for recommending movies. The online app was created to demonstrate the  User-based Collaborative Filtering approach for recommendation model.
